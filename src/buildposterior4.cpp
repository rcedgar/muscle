...
vector<float>* PairHMM::BuildPosterior4(
  ...
  const vector<SparseMx *>& sparseMxs) const
	{
	Die("TODO");
	...
	for (int PairIndex = 0; PairIndex < (int) PairCount; ++PairIndex)
		...
		const int ColCountSeq1 = Seq1->length;
		const int ColCountSeq2 = Seq2->length;

		asserta(ColCountSeq1 == ColCount1);
		asserta(ColCountSeq2 == ColCount2);

		const vector<int>& PosToCol1 = *Seq1->GetPosToCol();
		const vector<int>& PosToCol2 = *Seq2->GetPosToCol();
